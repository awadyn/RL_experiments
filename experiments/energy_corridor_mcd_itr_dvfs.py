# EXPERIMENT 4
# dataset: linux mcd logs + 2 qps values, varying itr-delay and dvfs values
import gym
...
class EnergyCorridor(gym.Env):
	def __init__(self, config):
		df = config["df"]
		self.reset_count = 0
		state_dict, reward_dict, action_dict, knob_list, key_list = utils.init_compact_dataset(df)	
		# for the purpose of this experiment: varying itr-delay and dvfs
	...
	def step(self, action):
		...
		new_key[0] = new_itr
		new_key[1] = new_dvfs
		# NOTE: target_qps index value is left unchanged
		new_key = tuple(new_key)
		# check if new key exists
		if (new_key not in self.key_space):
			print("KEY NOT FOUND: ", new_key)
			return self.state_space[self.cur_key], 0, False, {'error': 1} 
		new_energy = self.reward_space[new_key]['joules']
		done = (new_key == self.goal_key)
		#reward = 1.0 if done else -0.1
		reward = 0
		if (self.reward_space[new_key]['joules'] < self.reward_space[self.cur_key]['joules']):
			reward += 0.1
		else:
			reward -= 0.5
		if done:
...
df = pd.read_csv('../log_data/ebbrt_features_28.csv', sep = ' ')
